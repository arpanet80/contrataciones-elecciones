...
@Component({
  ...
  styleUrl: './procesos-nuevo.component.css'
})

export class ProcesosNuevoComponent implements OnInit, AfterViewInit {
  ...
  inicializaStepper() {
    ...
    stepper.on("kt.stepper.next", function (stepper) {
      // console.log("Ejecuta Antes del siguiente");
      stepper.goNext();
    });
  ...
  guardarProceso() {
    ...
    if (!this.formIniciofContrat.invalid) {
      ...
      // console.log('unidadOrg', unidadOrg);
      if (unidadOrg) {
    ...
    /*else {
      console.log('Formulario enviado:', this.formIniciofContrat.value);
    }*/
  }
  ...
  stepConitnuar() {
    // console.log("Valores del formulario:", this.formIniciofContrat.value);
    // console.log('Campos inválidos:', this.obtenerCamposInvalidos(this.formIniciofContrat));
    const tipoProceso = this.combosProceso.tipoproceso.find(tp => tp.id === this.formIniciofContrat.get('idtipoproceso')?.value);
    const formacontrat = this.combosProceso.formaContrat.find(tp => tp.id === this.formIniciofContrat.get('idformacontratacion')?.value);
    const formacadju = this.combosProceso.formaAdjudic.find(tp => tp.id === this.formIniciofContrat.get('idformaadjudic')?.value);
    ...
    this.resumenArray = [
      ...
      { nombre: 'Precio total:', valor:  this.formIniciofContrat.get('preciototal')?.value},
      { nombre: 'Plazo entrega:', valor:  this.formIniciofContrat.get('plazoentrega')?.value},
      { nombre: 'Objeto de contratacion:', valor:  this.formIniciofContrat.get('objetocontratacion')?.value},
      { nombre: 'Forma de contratacion:', valor:  formacontrat ? formacontrat.detalle : '' }, 
    ...
    if (this.formIniciofContrat.get('idtipoproceso')?.value == 3) {
      this.resumenArray.push(
      { nombre: 'Nivel salarial:', valor:  nivelsalarial ? nivelsalarial.nivelsalarial : '' }, 
      { nombre: 'Honorario mensual:', valor:  this.formIniciofContrat.get('honorariomensual')?.value},
      { nombre: 'Numero de casos:', valor:  this.formIniciofContrat.get('numerocasos')?.value},
      { nombre: 'Observaciones:', valor:  this.formIniciofContrat.get('observaciones')?.value},
      );
    }
  ...
  stepSubmitModal() {
    ...
    //Llena contenido base par las especificaciones

    if (this.especificacionesText == '') {
      let num = 1;
      let richtext = '';
      for (let item of this.dataSourceItems.data) {
        richtext += `<p><strong>${num}. ${item['requerimiento']}</strong></p><ul><li>Escriba especificaciones</li><li></li><li></li></ul>`;
        num++;
      }

      this.formIniciofContrat.patchValue({
        especificaciones: richtext
      });
    }

    // if (this.especificacionesFilled == false) {
  ...
  stepSiguinteModal() {
    ...
    /*this.dataSourceItems.data = this.selectedRowsRequerimientosPlan.map(item => {
      return new ItemsProceso(
        ...
        item.preciounitario || 0, // Lo mismo para precio unitario
        (item.cantidad || 0) * (item.preciounitario || 0), // Cálculo de total
        // item.idunidadorganizacional || 0, // Asumiendo que este campo se mapea de idunidadorganizacional
        item.requerimiento || '', // Asumiendo que requerimiento es el mismo campo en ambos modelos
        false // Valor por defecto para isEdit
      );
    });*/
